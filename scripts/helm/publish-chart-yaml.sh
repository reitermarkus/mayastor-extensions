...
# Requires a 'git fetch origin' (origin being the remote entry for openebs/mayastor-extensions) or equivalent.
...
main_branch_chart_app_version_from_latest_release_branch_name() {
...
branch_chart_version()
{
  ...
  if [ "$CHECK_BRANCH" == "develop" ]; then
    ...
    main_branch_chart_app_version_from_latest_release_branch_name "$LATEST_RELEASE_BRANCH"
